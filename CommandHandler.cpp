...
int getCurrBSSID(const uint8_t command[], uint8_t response[])
{
  ...
  uint8_t bssidLen = strlen(bssid);

  response[2] = 1; // number of parameters
  response[3] = bssidLen; // parameter 1 length

  memcpy(&response[4], bssid, bssidLen);
...
{
  uint8_t encryptionType = 0; //WiFi.encryptionType();
...
// TODO: Remove IP, as it was only used for UDP-Multicast
// NOTE: TCP used to be a uint8_t, but new version is void, so it has been moved out of the if statement.
...
// NOTE: Changed ip from uint32_t to IPAddress
...
{
  const char* bssid = WiFi.BSSIDstr(command[4]).c_str();
  uint8_t bssidLen = strlen(bssid);
...
// TODO: Not implemented. Removed since enterprise don't work in arduino.
...
int ping(const uint8_t command[], uint8_t response[])
{
  ...
  if (Ping.ping(ip, count)) {
      result = (uint16_t)Ping.averageTime();
  }
...
const CommandHandlerType commandHandlers[] = {
  ...
  // 0x30 -> 0x3f
  disconnect, NULL, getIdxRSSI, getIdxEnct, reqHostByName, getHostByName, NULL, getFwVersion, NULL, sendUDPdata, getRemoteData, getTime, getIdxBSSID, getIdxChannel, ping, getSocket,
...
// NOTE: I think this is the same as the GPIO0 thing from the depricated functions.
...
int CommandHandlerClass::handle(const uint8_t command[], uint8_t response[])
{
  ...
  if (responseLength == 0) {
    // Package start. 0x0F == command failed.
    response[0] = 0xef;
    response[1] = 0x00;
    // Package end. 0xEE == end of package
    response[2] = 0xee;

    responseLength = 3;
  } else {
    // Package start. 0x0E == command proccesed.
    response[0] = 0xe0;
    // ?? Command ??
    // TODO: I have removed 0x70-0x7f, do I need to change the 0x80 to 0x90?
